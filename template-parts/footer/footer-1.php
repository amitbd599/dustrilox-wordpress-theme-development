...
<footer>
    <div class="footer__area-2 black-bg-2 ">
        <?php if ( is_active_sidebar('footer-1-1') OR is_active_sidebar('footer-1-2') OR is_active_sidebar('footer-1-3') OR is_active_sidebar('footer-1-4') ): ?>
        <div class="pt-100" data-bg-color="<?php print esc_attr( $bg_color );?>">
            <div class="container">
                <div class="row">
                    <?php
                    if ( $footer_columns < 4 ) {
                    print '<div class="col-xl-4 col-lg-6 col-md-12 col-sm-12">';
                    dynamic_sidebar( 'footer-1-1' );
                    print '</div>';

                    print '<div class="col-xl-3 col-lg-6 col-md-6 col-sm-6">';
                    dynamic_sidebar( 'footer-1-2' );
                    print '</div>';

                    print '<div class="col-xl-2 col-lg-6 col-md-6 col-sm-6">';
                    dynamic_sidebar( 'footer-1-3' );
                    print '</div>';

                    print '<div class="col-xl-3 col-lg-6 col-md-8 col-sm-12">';
                    dynamic_sidebar( 'footer-1-4' );
                    print '</div>';
                    } else {
                        for ( $num = 1; $num <= $footer_columns; $num++ ) {
                            if ( !is_active_sidebar( 'footer-1-' . $num ) ) {
                                continue;
                            }
                            print '<div class="' . esc_attr( $footer_class[$num] ) . '">';
                            dynamic_sidebar( 'footer-1-' . $num );
                            print '</div>';
                        }
                    }
                ?>
